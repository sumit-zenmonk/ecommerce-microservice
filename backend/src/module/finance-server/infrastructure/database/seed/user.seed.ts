...
import { financeDataSource, options } from '../data-source';
...
async function create() {
    financeDataSource.setOptions({
        ...options,
    });

    await financeDataSource.initialize();

    const bcryptService = new BcryptService();
    const hashedPassword = await bcryptService.hashPassword("123");

    const queryRunner = financeDataSource.createQueryRunner();
    ...
    try {
        ...
        await financeDataSource.destroy();
    }
}
